...
(function (tn) {
    ...
    function mapOrderBookItem(ticket, action, orderBookItem) {
        return {
            ...
            orderAction: orderBookItem.s == "S" ? 3 /* Sell */ : 1 /* Buy */
        };
    }
    function mapOrderBook(orderBook) {
        //https://github.com/tradernet/tn.api#notifyOrderBook
        var res = orderBook.dom.map(function (m) {
            var ins = m.ins.map(function (x) { return mapOrderBookItem(m.i, 0 /* insert */, x); });
            var upd = m.upd.map(function (x) { return mapOrderBookItem(m.i, 1 /* update */, x); });
            var del = m.del.map(function (x) { return mapOrderBookItem(m.i, 2 /* remove */, x); });
